...
export function SeriesRunnerShowcase({ products }: SeriesRunnerShowcaseProps) {
  return (
    <section className="section-frame relative mt-6 overflow-hidden p-3 sm:mt-8 sm:p-5 lg:p-6">
      <div className="grid gap-3 sm:gap-4">
        {products.map((product) => (
          <div
            key={product.slug}
            className="relative overflow-hidden rounded-[24px] border border-white/10 bg-black/20 sm:rounded-[30px]"
          >
            <div className="relative aspect-[5/4] overflow-hidden sm:aspect-[16/8.4]">
              <Image
                src={product.runnerImageUrl}
                alt={`${product.color} Series 01 runner editorial`}
                fill
                sizes="100vw"
                className="object-cover object-[62%_center] sm:object-[62%_center]"
              />
