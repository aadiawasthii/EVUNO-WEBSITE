...
export function ProductCard({ product }: ProductCardProps) {
  return (
    <Link href={`/product/${product.slug}`} className="group block h-full">
      <article className="section-frame flex h-full flex-col p-4 transition duration-300 group-hover:border-white/16 sm:p-5">
        <div className="rounded-[28px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] p-3">
          <ProductModelViewer
            alt={product.name}
        ...
        <div className="mt-5 flex flex-1 flex-col">
          ...
          <h3 className="mt-4 text-[1.8rem] uppercase leading-[1.02] tracking-[0.12em] text-mist transition group-hover:text-chrome sm:text-[1.95rem]">
            {product.name}
          </h3>

          <p className="mt-4 min-h-[5rem] text-sm leading-7 text-steel">{product.tagline}</p>
